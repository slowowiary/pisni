...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #volume-row span { white-space: nowrap; min-width: 2.2rem; text-align: right; font-size: .8rem; color: #888; }
    #volume-slider {
      flex: 1; -webkit-appearance: none; appearance: none;
      height: 4px; border-radius: 2px;
      background: linear-gradient(to right, #3f51b5 var(--vol, 80%), #dde3ff var(--vol, 80%));
      outline: none; cursor: pointer;
    }
    #volume-slider::-webkit-slider-thumb {
      -webkit-appearance: none; width: 18px; height: 18px;
      border-radius: 50%; background: #3f51b5;
      box-shadow: 0 1px 4px rgba(63,81,181,.4); cursor: pointer;
    }
    #volume-slider::-moz-range-thumb {
      width: 18px; height: 18px; border: none;
      border-radius: 50%; background: #3f51b5; cursor: pointer;
    }
    ...
    #lyrics {
      ...
      /* GPU layer for smooth scroll on older Android */
    ...
    .word {
      ...
      position: relative;
      text-shadow: none;
      transition: color 0.4s ease, text-shadow 1.2s ease, box-shadow 1.2s ease;
    }
    ...
    .word.done { color: #bbb; box-shadow: none; text-shadow: none; }

    /* Верхня і нижня лінії через box-shadow (тільки top+bottom) */
    .word.pre-active {
      box-shadow:
        0 -2px 0 0 rgba(220, 0, 0, 0.50),
        0  2px 0 0 rgba(220, 0, 0, 0.50);
      text-shadow:
...
<body>
  ...
  <main>
    ...
    <div id="room-view" hidden style="width:100%;display:flex;flex-direction:column;align-items:center;gap:1.2rem">
      ...
      <!-- Volume control — visible to all, controlled by host -->
      <div id="volume-row" hidden>
        <span>🔊</span>
...
document.addEventListener('click', () => {
  ...
  <!-- NoSleep: запобігає гасінню екрана на iOS -->
